...
```python
agent.train(num_episodes=2000, plotting_interval=1000)
```

`train` 接受两个参数，参数 `num_episodes` 规定agent训练多少局，参数 `plotting_interval` 规定agent每多少局输出一次score、loss等指标的图像，并保存当前阶段的神经网络，默认1000。
