# FIXME: 然而并不会修
#  dist是torch.Size([128, 1, 51])
#  log_p = torch.log(dist[range(self.batch_size), action])会越界
import math
import random
...
from tetris.src.segment_tree import MinSegmentTree, SumSegmentTree
...
class Network(nn.Module):
    def __init__(
        ...
        结合了Noisy Net、Dueling DQN和Categorical DQN三种优化策略。第一个共享的特征层使用线性函数y=Wx+b，维度为128，随后分为优势头和状态价值头。优势头输出的是每个动作的优势值，状态价值头输出的是状态价值。两者都使用两层噪声网络，并采用值分布优化，输出维度分别扩大到out_dim * atom_size和atom_size，组合起来得到每个动作的动作价值分布。激活函数使用ReLU。

        Args:
            in_dim: 输入维度
            out_dim: 输出维度
            atom_size: 支点数量
            support: 支集模板（用于计算分布的向量模板）
        """
    ...
    def dist(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Q(s, a; w) = V(s; wv) + A(s, a; wa) - mean A(s, a; wa)

        在值分布优化下，每个Q实际上都要表示成概率分布的形式。而总体的Q需要通过zᵀp计算，这个步骤在forward()函数中完成。
...
class DQNAgent:
    ...
    Attributes:
        ...
        v_max (float): 离散支集的上限
        atom_size (int): 支点数量
        support (torch.Tensor): 支集模板（用于计算分布的向量模板）
    ...
    def select_action(self, state: np.ndarray, next_steps: Dict, next_states) -> np.ndarray:
        """根据当前状态选取动作。
    ...
    def step(self, action: np.ndarray) -> Tuple[np.float64, bool]:
        """采取动作并更新环境。
        ...
        """
        reward, done = self.env.step(action, render=True)
    ...
    def train(
        ...
        Args:
            ...
            eps: 最后20个回合的得分方差小于eps，可认为模型已收敛
        """
        self.is_test = False
        ...
        while episode < num_frames:
            ...
            # PER: increase beta
            fraction = min(episode / num_frames, 1.0)
    ...
    def _compute_dqn_loss(self, samples: Dict[str, np.ndarray],
        ...
        使用TD算法时，运用Double DQN策略，从DQN网络中选择动作a，在目标网络中计算Q，以缓解高估问题。计算loss时使用C51中提出的算法，将support'和atom'分别与原support和原atom对齐后，采用KL散度计算loss。最终以element-wise的形式返回loss，用于更新优先经验回放数组中的优先级。
    ...
    def _plot(
        ...
        print('frame %s. score: %s, loss: %s' % (frame_idx, np.mean(scores[-10:]), np.mean(losses[-10:])))
